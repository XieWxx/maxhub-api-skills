...
> ⚠️ **参数校验提示**：`video_id` 为必填参数，格式为纯数字字符串。`cursor` 使用上一页返回的值，不要自行构造。
...
> ⛔ **已废弃**：上游接口已停用（返回 400）。替代接口：`暂无替代接口`。请勿使用此接口。
...
请使用接口返回的 `decode_key` 字段和加密视频文件，通过第三方工具进行解密。
...
> ⚠️ **参数校验提示**：`url` 为必填参数，须为完整的微信公众号文章链接（含 `__biz`、`mid`、`idx`、`sn` 参数）。
...
> ⚠️ **参数校验提示**：`keyword` 为必填参数，长度 1-50 字符。搜索频率受限，建议间隔 5 秒以上。
