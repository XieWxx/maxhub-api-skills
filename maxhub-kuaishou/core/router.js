...
const INTENT_API_MAP = {
  search_video: ['searchComprehensive', 'searchVideoV2'],
  get_video_detail: ['fetchOneVideo', 'fetchOneVideoV2'],
  get_user_profile: ['fetchUserInfo', 'fetchOneUserV2'],
  get_user_videos: ['fetchUserPost', 'fetchUserPostV2', 'fetchUserHotPost'],
  get_comments: ['fetchOneVideoComment', 'fetchOneVideoSubComment'],
  get_hot_list: ['fetchKuaishouHotListV1', 'fetchKuaishouHotListV2'],
  search_user: ['searchUserV2'],
};
...
const router = {
  ...
  routes: {
    ...
    async search_video({ keyword, page = 1, count = 20 }) {
      ...
      const apiName = decision.selected?.apiName || 'searchComprehensive';
    ...
    async get_video_detail({ photo_id, page = 1, count = 20 }) {
      const decision = router.decideBestApi('get_video_detail', { photo_id });
      const apiName = decision.selected?.apiName || 'fetchOneVideo';
      const result = await api[apiName]({ photo_id });
      return {
        success: true,
    ...
    async get_user_videos({ user_id, page = 1, count = 20 }) {
      const decision = router.decideBestApi('get_user_videos', { user_id });
      const apiName = decision.selected?.apiName || 'fetchUserPost';
      const result = await api[apiName]({ user_id });
      return {
        success: true,
    ...
    async get_comments({ photo_id, page = 1, count = 20 }) {
      const decision = router.decideBestApi('get_comments', { photo_id });
      const apiName = decision.selected?.apiName || 'fetchOneVideoComment';
      const result = await api[apiName]({ photo_id });
      return {
        success: true,
        intent: 'get_comments',
