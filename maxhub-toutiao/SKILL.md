...
头条数据采集与分析——头条、toutiao、新闻等平台数据的智能采集与分析工具，支持视频搜索、用户分析、热门趋势追踪等能力，用自然语言即可获取数据。

## 功能亮点

- 智能识别：根据自然语言自动匹配最合适的API
- 链式调用：复杂需求自动串联多个API完成
- 全量覆盖：共 7 个API，覆盖数据采集、搜索查询、用户分析等场景
...
直接输入：头条、toutiao、新闻、资讯、今日头条

### 使用示例

1. 示例：搜索今日头条上的科技新闻 → 返回新闻列表，包含标题、来源、评论数
...
| API | 方法 | 必填参数 | 说明 |
|:---|:---|:---|:---|
| `web/get_article_info` | GET | - | 获取指定文章的信息 |
| `web/get_video_info` | GET | - | 获取指定视频的信息 |

**Toutiao App**（5个API）

| API | 方法 | 必填参数 | 说明 |
|:---|:---|:---|:---|
| `app/get_user_id` | GET | - | 从头条用户主页获取用户user_id |
| `app/get_comments` | GET | - | 获取指定作品的评论 |
| `app/get_article_info` | GET | - | 获取指定文章的信息 |
| `app/get_user_info` | GET | - | 获取指定用户的信息 |
| `app/get_video_info` | GET | - | 获取指定视频的信息 |
...
v1.0.1 V2架构升级，全量API覆盖，兼容层设计，场景化展示
