...
Skill: `maxhub-xigua` · Current version: `3.7.2` · Owner: `maxhub-skills`
...
### 3.1 ClawHub（国际版主源）
```bash
clawhub upgrade maxhub-xigua
```

### 3.2 SkillHub（国内加速镜像）
```bash
skillhub upgrade maxhub-xigua
```
Web 入口：https://skillhub.cn/skills/maxhub-xigua

### 3.3 GitHub（降级方案）
```bash
cd ~/.openclaw/skills/maxhub-xigua && git pull origin main
```
...
### 场景 A：合法路径返回 404
> "路径 `<path>` 在白名单中合法，但上游返回 404。建议更新：`skillhub upgrade maxhub-xigua`"

### 场景 B：用户询问版本
> "本 SKILL 当前版本：**v3.7.2**。检查最新版本：`skillhub info maxhub-xigua`"
