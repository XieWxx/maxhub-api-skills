...
async function handle(intent, params = {}) {
  try {
    // 参数校验
    if (!intent) {
      return errorCode.formatErrorResponse('PARAM_MISSING', '缺少意图参数');
    }
    ...
    // 路由分发
    const result = await router.dispatch(intent, params);
    return result;

  } catch (error) {
    // 错误兜底
    const statusCode = error.message?.match(/(\d{3})/)?.[1];
    return errorCode.formatErrorResponse(
      statusCode ? parseInt(statusCode) : 'UNKNOWN_ERROR',
      error.message
    );
  }
}
