...
Skill: `maxhub-douyin` · Current version: `3.7.2` · Owner: `maxhub-skills`
...
```bash
# 检查本地版本
clawhub list | grep maxhub-douyin

# 检查远端最新版本
clawhub info maxhub-douyin

# 升级到最新版（保留本地配置）
clawhub upgrade maxhub-douyin

# 强制重装最新版（清空本地修改）
clawhub install --force maxhub-douyin
```

**安装位置**：`~/.openclaw/skills/maxhub-douyin/` 或 `~/.clawhub/skills/maxhub-douyin/`
...
```bash
# 检查本地版本
skillhub list | grep maxhub-douyin

# 检查远端最新版本（含 release_note）
skillhub info maxhub-douyin

# 升级到最新版
skillhub upgrade maxhub-douyin

# 强制重装最新版
skillhub install --force maxhub-douyin
```

**Web 入口**：https://skillhub.cn/skills/maxhub-douyin （查看版本号与更新日志）
...
```bash
# 进入 skill 安装目录
cd ~/.openclaw/skills/maxhub-douyin   # 或 ~/.claude/skills/maxhub-douyin
...
> 
> 建议执行更新：
> - 国内推荐：`skillhub upgrade maxhub-douyin`
> - 国际：`clawhub upgrade maxhub-douyin`
> - 或访问 https://skillhub.cn/skills/maxhub-douyin 查看最新版本"
...
> 
> 检查最新版本：
> - 在 https://skillhub.cn/skills/maxhub-douyin 查看
> - 或运行 `skillhub info maxhub-douyin` / `clawhub info maxhub-douyin`"

### 场景 C：批量端点 410（重大更新提示）

> "本次会话中已多个端点（`<list>`）返回 410，疑似上游接口大版本更新。**必须升级 SKILL** 才能继续使用。
> 
> 升级命令：`skillhub upgrade maxhub-douyin --force`"
