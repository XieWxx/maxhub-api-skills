...
Skill: `maxhub-threads` · Base URL: `https://www.aconfig.cn` · Version: `3.7.2`
...
1. **路径禁止臆造**：所有路径必须逐字符匹配 [`endpoints_whitelist.yaml`](./endpoints_whitelist.yaml)；不在清单中 → **STOP**，禁止"换一个版本试试"、"改一改路径段试试"、"把 fetch_post_detail 改成 fetch_post_detail_v3 试探"。
2. **404 / 400 必须先做防臆造自检**：见 [§ 3.1](#-31-防臆造自检清单404--400-处理前置步骤)，绕过自检直接重试或改路径都属于违反防臆造红线。
3. **读取端点 5xx 重试 ≤ 1 次**：避免重复扣配额。
4. **找不到能力必须 STOP 并告知用户**：用户请求 Threads 不支持的能力（见下方"不支持能力清单"）→ 直接告知该 skill 不支持，**禁止**用其他端点拼凑伪造结果。
5. **username / user_id / post_id 不可臆造**：`username` 是 Threads URL slug，`user_id` 是纯数字，`post_id` 是纯数字；Agent 不可自行编造。
...
10. **fetch_post_detail_v2 的 post_id 与 url 至少提供一个**：两者都不传会触发 422；同时传时 post_id 优先。

---

## 0.1 ❌ 本 skill 不支持的能力 (Out of Scope)

> 用户请求以下功能时，**直接告知不支持并 STOP**。**禁止**用现有 11 个端点"拼凑"或"模拟"以下能力：

| 不支持的能力 | 说明 |
|------------|------|
| 发布/编辑/删除帖子 | 无写入端点 |
| 点赞 / 取消点赞 | 无社交互动写入端点 |
| 转发 / 引用帖子操作 | 仅支持读转发列表，无写入端点 |
| 评论 / 删评论 | 仅支持读评论，无写入端点 |
| 关注 / 取消关注 | 无写入端点 |
| 发送私信 / DM | 无私信端点 |
| 修改用户资料 / 头像 | 无修改端点 |
...
> 遇到上述请求 → 直接回复用户："本 skill 暂不支持此能力，建议在 Threads 官方应用中操作"，**禁止**用 fetch_user_info / search_top 等端点伪造结果。
...
|----|----------|---------------|--------|------|
| fetch_user_info | 用 username 取用户信息（**链式起点**） | user.md | GET | low |
| fetch_user_info_by_id | 用 user_id 取用户信息 | user.md | GET | low |
| fetch_user_posts | 用 user_id 取用户帖子列表 | user.md | GET | low |
| fetch_user_reposts | 用 user_id 取用户转发列表 | user.md | GET | low |
| fetch_user_replies | 用 user_id 取用户回复列表 | user.md | GET | low |
| fetch_post_detail | 用 post_id 取帖子详情（纯数字 ID） | content.md | GET | low |
| fetch_post_detail_v2 | 用短代码/URL 取帖子详情（**推荐**） | content.md | GET | low |
| fetch_post_comments | 用 post_id 取帖子评论 | content.md | GET | low |
| search_top | 搜索热门内容（**内容冷启动入口**） | content.md | GET | low |
| search_recent | 搜索最新内容 | content.md | GET | low |
| search_profiles | 搜索用户档案 | content.md | GET | low |
...
### `username` — Threads 用户名（URL slug）
- **产出**：用户直接提供 / `search_profiles` → `$.data.users[].username`
- **输入**：`fetch_user_info`

### `user_id` — 用户 ID（纯数字，如 `67027868801`）
- **可从这些端点产出（OUT）**：
  - `fetch_user_info` → `$.data.pk`
  - `fetch_user_info_by_id` → `$.data.pk`
  - `search_profiles` → `$.data.users[].pk`
  - `fetch_post_detail` → `$.data.user.pk`（帖子作者）
  - `fetch_post_detail_v2` → `$.data.user.pk`（帖子作者）
  - `fetch_post_comments` → `$.data.comments[].user.pk`（评论者）
- **可作为输入（IN）**：
...
### `post_id` — 帖子 ID（纯数字，如 `3349029093483693129`）
- **可从这些端点产出（OUT）**：
  - `fetch_user_posts` → `$.data.threads[].id`
  - `fetch_post_detail` → `$.data.id`
  - `fetch_post_detail_v2` → `$.data.post_id`
  - `search_top` → `$.data.threads[].id`
  - `search_recent` → `$.data.threads[].id`
- **可作为输入（IN）**：
  - `fetch_post_detail`（纯数字 ID）
  - `fetch_post_comments`

### `post_id`（短代码）— 帖子短代码（如 `DPVUglOjOUu`）
- **产出**：用户从 Threads URL 中提取（如 `https://www.threads.com/@user/post/DPVUglOjOUu`）
- **输入**：`fetch_post_detail_v2` 的 `post_id` 参数

### `end_cursor` — 分页游标（通用）
- **产出/输入**：所有支持分页的端点（同端点的下一次调用使用上次响应的 `$.data.next_cursor`，并以 `$.data.has_more` 判定是否继续翻页）
...
1. **路径白名单逐字符比对**
   - 把刚才请求的完整路径（`/api/v1/threads/web/<id>`）逐字符与 [`endpoints_whitelist.yaml`](./endpoints_whitelist.yaml) 中的 `endpoints[].path` 对照
...
4. **资源 ID 来源溯源**
   - 请求中的 `username` / `user_id` / `post_id` 等是否真实来自之前某个端点的**响应字段**（json_path 可追溯）？
...
2. **必填项齐全**
   ...
   - `oneOf(post_id, url)` 类型是否做到"至少传一个"？
3. **类型与格式严格匹配**
...
### 降级策略原则
1. **版本降级**：`fetch_post_detail_v2` 失败 → 尝试 `fetch_post_detail`（需纯数字 post_id）
2. **数据完整度降级**：详情接口失败 → 列表接口取相同字段
3. **维度降级**：用户详情失败 → `search_profiles` 取概要
...
| 首选端点 | 替代端点 | 替代条件 | 数据差异 |
|---------|---------|---------|---------|
| fetch_post_detail_v2 | fetch_post_detail | 已有纯数字 post_id | V2 支持 URL/短代码；V1 仅支持纯数字 ID |
| fetch_post_detail | fetch_post_detail_v2 | 已有帖子短代码或 URL | V1 仅支持纯数字 ID；V2 更灵活 |
| fetch_user_info | search_profiles | username 足够独特 | search_profiles 返回字段较少，可能命中重名 |
| fetch_user_info_by_id | fetch_user_info（已知 username） | 已有 username | 需额外一步获取 username |
| search_top | search_recent | 需要内容但热门搜索失败 | 结果按时间排序，非热门排序 |
| search_recent | search_top | 需要内容但最新搜索失败 | 结果按热门排序，非时间排序 |
| fetch_user_posts | 无替代 | — | 失败 STOP |
| fetch_user_reposts | 无替代 | — | 失败 STOP |
| fetch_user_replies | 无替代 | — | 失败 STOP |
| fetch_post_comments | 无替代 | — | 评论无替代来源，失败直接 STOP |
| search_profiles | 无替代 | — | 用户搜索无替代 |
...
### 更新通道（按优先级）
1. **国内首选 ⭐⭐⭐**：`skillhub upgrade maxhub-threads`
2. **国际主源 ⭐⭐⭐**：`clawhub upgrade maxhub-threads`
