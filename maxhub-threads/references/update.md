...
Skill: `maxhub-threads` · Current version: `3.7.2` · Owner: `maxhub-skills`
...
1. **被动触发（最高优先级）**
   ...
   - 多个端点连续返回 410（路由批量下线）
   - 上游响应字段结构与 reference OUT 表明显不一致（如 `$.data.pk` 改名）
...
```bash
# 检查本地版本
skillhub list | grep maxhub-threads

# 检查远端最新版本（含 release_note）
skillhub info maxhub-threads

# 升级到最新版
skillhub upgrade maxhub-threads

# 强制重装最新版
skillhub install --force maxhub-threads
```

**Web 入口**：https://skillhub.cn/skills/maxhub-threads （查看版本号与更新日志）
...
```bash
# 检查本地版本
clawhub list | grep maxhub-threads

# 检查远端最新版本
clawhub info maxhub-threads

# 升级到最新版（保留本地配置）
clawhub upgrade maxhub-threads

# 强制重装最新版（清空本地修改）
clawhub install --force maxhub-threads
```

**安装位置**：`~/.openclaw/skills/maxhub-threads/` 或 `~/.clawhub/skills/maxhub-threads/`
...
```bash
# 进入 skill 安装目录
cd ~/.openclaw/skills/maxhub-threads   # 或 ~/.claude/skills/maxhub-threads
...
> ```bash
> pwd && ls -la ~/.openclaw/skills/maxhub-threads
> ```
> 2. 把旧目录**重命名备份**（避免破坏性删除）：
> ```bash
> mv ~/.openclaw/skills/maxhub-threads ~/.openclaw/skills/maxhub-threads.bak.$(date +%s)
> ```
> 3. 重新克隆：
> ```bash
> cd ~/.openclaw/skills && \
>   git clone https://github.com/XieWxx/maxhub-api-skills.git maxhub-threads
> ```
>
> 备份目录在确认新版本工作正常后再删除。
```
...
>
> 建议执行更新：
> - 国内推荐：`skillhub upgrade maxhub-threads`
> - 国际：`clawhub upgrade maxhub-threads`
> - 或访问 https://skillhub.cn/skills/maxhub-threads 查看最新版本"
...
>
> 检查最新版本：
> - 在 https://skillhub.cn/skills/maxhub-threads 查看
> - 或运行 `skillhub info maxhub-threads` / `clawhub info maxhub-threads`"

### 场景 C：批量端点 410（重大更新提示）

> "本次会话中已多个端点（`<list>`）返回 410，疑似上游接口大版本更新。**必须升级 SKILL** 才能继续使用。
>
> 升级命令：`skillhub upgrade maxhub-threads --force`"
...
- **用户对自有数据的请求**（如查询一个不存在的 username/user_id）：与 SKILL 版本无关
