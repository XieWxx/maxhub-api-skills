...
Skill: `maxhub-xiaohongshu` · Current version: `3.7.2` · Owner: `maxhub-skills`
...
1. **被动触发（最高优先级）**
   ...
   - 多个端点连续返回 410（路由批量下线）
   - 上游响应字段结构与 reference OUT 表明显不一致（如 `$.data.note_id` 改名）
...
## 3. 通过 SkillHub / ClawHub / GitHub 更新

本 skill 同时发布于多个 registry。Agent 在检测到需要更新时，**优先**指引用户使用本地已安装的 CLI；若两者都不可用，再降级到手动 git 拉取。

### 3.1 SkillHub（推荐 ⭐⭐⭐ 国内加速镜像）

**适用环境**：中国大陆用户、ClawHub 下载慢、需要安全审计版本。

```bash
# 检查本地版本
skillhub list | grep maxhub-xiaohongshu

# 检查远端最新版本（含 release_note）
skillhub info maxhub-xiaohongshu

# 升级到最新版
skillhub upgrade maxhub-xiaohongshu

# 强制重装最新版
skillhub install --force maxhub-xiaohongshu
```

**Web 入口**：https://skillhub.cn/skills/maxhub-xiaohongshu （查看版本号与更新日志）
...
### 3.2 ClawHub（推荐 ⭐⭐⭐ 国际版主源）

**适用环境**：用户已安装 ClawHub CLI（`clawhub` 命令）。

```bash
# 检查本地版本
clawhub list | grep maxhub-xiaohongshu

# 检查远端最新版本
clawhub info maxhub-xiaohongshu

# 升级到最新版（保留本地配置）
clawhub upgrade maxhub-xiaohongshu

# 强制重装最新版（清空本地修改）
clawhub install --force maxhub-xiaohongshu
```

**安装位置**：`~/.openclaw/skills/maxhub-xiaohongshu/` 或 `~/.clawhub/skills/maxhub-xiaohongshu/`
...
```bash
# 进入 skill 安装目录
cd ~/.openclaw/skills/maxhub-xiaohongshu   # 或 ~/.claude/skills/maxhub-xiaohongshu
...
> ```bash
> pwd && ls -la ~/.openclaw/skills/maxhub-xiaohongshu
> ```
> 2. 把旧目录**重命名备份**（避免破坏性删除）：
> ```bash
> mv ~/.openclaw/skills/maxhub-xiaohongshu ~/.openclaw/skills/maxhub-xiaohongshu.bak.$(date +%s)
> ```
> 3. 重新克隆：
> ```bash
> cd ~/.openclaw/skills && \
>   git clone https://github.com/XieWxx/maxhub-api-skills.git maxhub-xiaohongshu
> ```
>
> 备份目录在确认新版本工作正常后再删除。
```
...
> 
> 建议执行更新：
> - 国内推荐：`skillhub upgrade maxhub-xiaohongshu`
> - 国际：`clawhub upgrade maxhub-xiaohongshu`
> - 或访问 https://skillhub.cn/skills/maxhub-xiaohongshu 查看最新版本"
...
> 
> 检查最新版本：
> - 在 https://skillhub.cn/skills/maxhub-xiaohongshu 查看
> - 或运行 `skillhub info maxhub-xiaohongshu` / `clawhub info maxhub-xiaohongshu`"

### 场景 C：批量端点 410（重大更新提示）

> "本次会话中已多个端点（`<list>`）返回 410，疑似上游接口大版本更新。**必须升级 SKILL** 才能继续使用。
> 
> 升级命令：`skillhub upgrade maxhub-xiaohongshu --force`"
...
- **用户对自有数据的请求**（如查询一个不存在的 note_id / user_id）：与 SKILL 版本无关
