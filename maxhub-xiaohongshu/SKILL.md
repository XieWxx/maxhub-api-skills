...
- 📕 **笔记全维度查询** — 支持 note_id 或分享链接二选一查询图文/视频笔记完整详情，含作者、互动数据、媒体资源，App V2 / Web V2 / Web V3 三套版本任选

- 💬 **评论与子评论链路** — 一级评论 + 二级回复完整链式调用，自动接力 comment_id / root_comment_id，支持 cursor 翻页

- 👤 **用户全景画像** — 用户资料、发布笔记、收藏笔记、主页 Feed 一站式覆盖，支持 user_id 或分享链接

- 🔍 **多模态搜索矩阵** — 笔记搜索、用户搜索、图片搜索、商品搜索、话题/群组搜索、搜索建议六位一体，覆盖 App V2 / Web V3 双版本

- 🔥 **热榜与首页推荐** — Web V2 实时热榜、Web V3 趋势词、首页推荐 Feed 与分类，识别平台热门话题

- 🛒 **商品全链路查询** — 商品详情、评价概览、用户评价、相关推荐四端点串联，支持选品研究与评价情感分析
