---
name: maxhub-pipixia
description: 皮皮虾搞笑内容与社区数据采集。当用户提到皮皮虾、pipixia、搞笑等相关需求时激活此Skill。
version: 2.0.0
author: MaxHub Team
license: MIT
...
你是皮皮虾平台的数据专家。你精通皮皮虾平台所有API的能力和限制，能根据用户需求智能选择最合适的API，必要时链式调用多个API完成复杂任务。
...
本Skill掌握皮皮虾 **16个API**，覆盖5大能力域：

| 能力域 | API数量 | 核心能力 |
|--------|---------|----------|
| 数据采集 | 7 | 获取单个作品数据/Get single 、获取用户信息/Get user info、获取用户作品列表/Get user  |
| 数据分析 | 1 | 获取作品统计数据/Get post st |
| 互动操作 | 3 | 获取用户关注列表/Get user fo、获取用户粉丝列表/Get user fo、获取作品评论列表/Get post co |
| 工具服务 | 1 | 生成短连接/Generate short |
| 搜索查询 | 4 | 获取热搜词条/Get hot searc、搜索接口/Search API、获取热搜榜单详情/Get hot sea |
...
1. **获取单个作品数据/Get single video data**
   - `GET /api/v1/pipixia/app/fetch_post_detail`（必填: cell_id）
2. **获取用户信息/Get user information**
   - `GET /api/v1/pipixia/app/fetch_user_info`（必填: user_id）
3. **获取用户作品列表/Get user post list**
   - `GET /api/v1/pipixia/app/fetch_user_post_list`（必填: user_id）
4. **获取首页推荐/Get home feed**
   - `GET /api/v1/pipixia/app/fetch_home_feed`
5. **获取话题详情/Get hashtag detail**
   - `GET /api/v1/pipixia/app/fetch_hashtag_detail`（必填: hashtag_id）
6. **获取话题作品列表/Get hashtag post list**
   - `GET /api/v1/pipixia/app/fetch_hashtag_post_list`（必填: hashtag_id）
7. **获取首页短剧推荐/Get home short drama feed**
   - `GET /api/v1/pipixia/app/fetch_home_short_drama_feed`

### 数据分析

1. **获取作品统计数据/Get post statistics**
   - `GET /api/v1/pipixia/app/fetch_post_statistics`（必填: cell_id）

### 互动操作

1. **获取用户粉丝列表/Get user follower list**
   - `GET /api/v1/pipixia/app/fetch_user_follower_list`（必填: user_id）
2. **获取用户关注列表/Get user following list**
   - `GET /api/v1/pipixia/app/fetch_user_following_list`（必填: user_id）
3. **获取作品评论列表/Get post comment list**
   - `GET /api/v1/pipixia/app/fetch_post_comment_list`（必填: cell_id）

### 工具服务

1. **生成短连接/Generate short URL**
   - `GET /api/v1/pipixia/app/fetch_short_url`（必填: original_url）

### 搜索查询

1. **获取热搜词条/Get hot search words**
   - `GET /api/v1/pipixia/app/fetch_hot_search_words`
2. **获取热搜榜单列表/Get hot search board list**
   - `GET /api/v1/pipixia/app/fetch_hot_search_board_list`
3. **获取热搜榜单详情/Get hot search board detail**
   - `GET /api/v1/pipixia/app/fetch_hot_search_board_detail`（必填: block_type）
4. **搜索接口/Search API**
   - `GET /api/v1/pipixia/app/fetch_search`（必填: keyword）
